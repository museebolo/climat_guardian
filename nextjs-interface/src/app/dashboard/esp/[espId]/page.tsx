...
export default function Page({params}: { params: any }) {
    ...
    const handleDateChange = (newDate: DateRange | undefined) => {
        ...
        router.push(`/dashboard/esp/${params.espId}?precision=${precision}&from=${from}&to=${to}`);
    };
    ...
    const handleSelect = (value: any) => {
        setPrecision(value);
        router.push(`/dashboard/esp/${params.espId}?precision=${value}&from=${date?.from?.toISOString()}&to=${date?.to?.toISOString()}`);
    };
    ...
    return (
        <div className="flex h-full w-full min-w-[500px] flex-col gap-y-5 pt-2">
            ...
            <div className="flex flex-col gap-x-5 gap-y-5 sm:flex-row">
                <DateRangeElement date={date} setDate={(newDate: DateRange | undefined) => {
                    setDate(newDate);
